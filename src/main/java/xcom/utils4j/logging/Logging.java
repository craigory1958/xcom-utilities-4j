...
public class Logging {
	 ...
	 */
	@Before("execution(@xcom.utils4j.logging.annotations.Log * *(..))")
	 ...
	 */
	@AfterReturning(pointcut = "execution(@xcom.utils4j.logging.annotations.Log * *(..))", returning = "returned")
	 ...
	 *            - Contains method data
	 */
	@Before("execution(@xcom.utils4j.logging.annotations.Log *.new(..))")
	 ...
	 *            - Contains method data
	 */
	@AfterReturning(pointcut = "execution(@xcom.utils4j.logging.annotations.Log *.new(..))")
	 ...
	 *
	 * If a method it will execute a statement like: LOGGER.debug("[ EXIT] parseList(|{}|)", results) if there is a returnVal or: LOGGER.debug(
	 * "[ EXIT] parseList()"; if there is a void return value
	 *
			...
			switch ( joinType ) {
				...
				case Constructor:
					log(classLoggingLevel, annotatedMethodLoggingLevel, slf4jLogger, log4jLogger, "[ EXIT] {} ({})",
							formatClassNameForLogging(joinPoint.getThis().getClass()), joinPoint.getThis()) ;
					break ;
	...
	org.slf4j.event.Level annotatedMethodLoggingLevel(final JoinPoint joinPoint, final JoinTypes joinType) {
		...
		final org.slf4j.event.Level loggingLevel ;

		switch ( joinType ) {

			case Constructor:
				final Constructor<?> constructor = ((ConstructorSignature) joinPoint.getSignature()).getConstructor() ;
				loggingLevel = (constructor.getAnnotation(Log.class)).value()[0] ;
				break ;

			case Method:
				loggingLevel = ((MethodSignature) joinPoint.getSignature()).getMethod().getAnnotation(Log.class).value()[0] ;
				break ;
	...
	org.slf4j.event.Level annotatedParameterLoggingLevel(final JoinPoint joinPoint, final JoinTypes joinType) {
		...
		return (log.value().length > 1 ? log.value()[1] : org.slf4j.event.Level.TRACE) ;
	}


	void log(final org.slf4j.event.Level classLoggingLevel, final org.slf4j.event.Level loggingLevel, final org.slf4j.Logger slf4jLogger, final org.apache.log4j.Logger log4jLogger,
			final String msg, final Object... objects) {
	...
	/**
	 * Returns a readable string representation of the class that needs to be logged
	 *
	 * @param clazz
	 *            - Class object that we want to format its name for Logging
	 * @return Readable String
	 */
	static String formatClassNameForLogging(final Class<?> clazz) {

		String result = clazz.toString() ;

		if ( clazz.isArray() )
			result = Utility.signatureToString(clazz.getName()) ;
		else {
			// Strips off the x.y.z.Class to just Class
