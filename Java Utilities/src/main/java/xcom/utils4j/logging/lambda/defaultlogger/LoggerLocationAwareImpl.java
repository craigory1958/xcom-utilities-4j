...
import xcom.utils4j.logging.lambda.LoggerUtils ;
 ...
 */
public class LoggerLocationAwareImpl implements Logger, org.slf4j.spi.LocationAwareLogger {
	...
	public void doLog(final org.slf4j.Marker marker, final org.slf4j.event.Level level, final Supplier<String> msgSupplier, final Throwable t) {
		if ( LoggerUtils.isLogLevelEnabled(underlyingLogger, level, marker) )
	...
	public void doLog(final org.slf4j.Marker marker, final org.slf4j.event.Level level, final String format, final Supplier<?>[] argSuppliers,
			final Throwable t) {
		if ( LoggerUtils.isLogLevelEnabled(underlyingLogger, level, marker) )
			underlyingLogger.log(marker, getLoggerFQCN(), level.toInt(), format, LoggerUtils.argSuppliersToArgs(argSuppliers), t) ;
	}
