...
public class _Test_Query$Connected {
	...
	public void method_statement_Returns_CorrectType() {
		...
		Assert.assertTrue("Method 'statement()' did not return expected type.",
				new Query() {}.new Query$Connected(null).statement(TestQuerys_StatementWhereClause) instanceof Query$Stated) ;
	}
	...
	public void method_tableExists_Returns_True() throws SQLException {
		...
		final DatabaseMetaData dbmd = Mockito.mock(DatabaseMetaData.class) ;
		Mockito.when(dbmd.getTables(null, null, TestQuerys_TestTableName, null)).thenReturn(rs) ;

		final Connection connection = Mockito.mock(Connection.class) ;
		Mockito.when(connection.getMetaData()).thenReturn(dbmd) ;

		final Query$Connected connected = new Query() {}.new Query$Connected(connection) ;
		Assert.assertTrue("Incorrect value for  method 'tableExists()':", connected.tableExists(TestQuerys_TestTableName)) ;
	}
	...
	public void method_tableExists_Returns_False() throws SQLException {
		...
		final Query$Connected connected = new Query() {}.new Query$Connected(connection) ;
		Assert.assertFalse("Incorrect value for  method 'tableExists()':", connected.tableExists(TestQuerys_TestTableName)) ;
	}
	...
	public void method_tableExists_ThrowsOn_DatabaseMetaData$$getTables() throws SQLException {
		...
		final DatabaseMetaData dbmd = Mockito.mock(DatabaseMetaData.class) ;
		Mockito.when(dbmd.getTables(null, null, TestQuerys_TestTableName, null)).thenThrow(new SQLException()) ;

		final Connection connection = Mockito.mock(Connection.class) ;
		Mockito.when(connection.getMetaData()).thenReturn(dbmd) ;

		new Query() {}.new Query$Connected(connection).tableExists(TestQuerys_TestTableName) ;
	}
}
