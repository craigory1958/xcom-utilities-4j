...
import static xcom.utils4j.data.database.TestQuerys.TestQuerys_ValueValue_15000 ;
...
public class _Test_RowMapper_ToMap {
	...
	public void method_mapRow_Retures_ValidData() throws SQLException {
		...
		Mockito.when(md.getColumnLabel(Mockito.anyInt())).thenReturn(TestQuerys_KeyColumnName).thenReturn(TestQuerys_ValueColumnName) ;

		final ResultSet rs = Mockito.mock(ResultSet.class) ;
		Mockito.when(rs.getMetaData()).thenReturn(md) ;
		Mockito.when(rs.getString(Mockito.anyInt())).thenReturn(TestQuerys_KeyValue_15000).thenReturn(TestQuerys_ValueValue_15000) ;

		final RowMapper_ToMap rowMapper = new RowMapper_ToMap(new ColumnMapper_FromMySQL()) ;
		Assert.assertEquals("Incorrect value for method 'mapRow':", TestQuerys_StatementResults.get(0), rowMapper.mapRow(rs)) ;
	}
